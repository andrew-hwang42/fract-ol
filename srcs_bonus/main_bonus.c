...
/*   Updated: 2025/05/26 01:18:51 by ahwang           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	init_struct_mlx(t_mlx *mlx, int fractol_type)
{
	...
	else if (fractol_type == BURNING_SHIP)
		mlx->win = mlx_new_window(mlx->mlx_ptr, WIN_X, WIN_Y, "Burning_ship");
	if (!mlx->win)
		return (0);
...
int	main(int argc, char **argv)
{
	...
	if (!(check_args_mandelbrot(argc, argv, &fractol)
			...
			|| check_args_burning_ship(argc, argv, &fractol)))
	{
